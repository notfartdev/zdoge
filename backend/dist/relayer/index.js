 ...
 */
async function validateWithdrawal(poolAddress, proof, root, nullifierHash, recipient, fee) {
    try {
 ...
 */
async function submitWithdrawal(poolAddress, proof, root, nullifierHash, recipient, fee) {
...
app.post('/withdraw', async (req, res) => {
    ...
    let parsedFee;
    try {
        parsedProof = proof.map((p) => BigInt(p));
    ...
    // Validate withdrawal
    const validation = await validateWithdrawal(poolAddress, parsedProof, root, nullifierHash, recipient, parsedFee);
    if (!validation.valid) {
    ...
    try {
        ...
        const { txHash } = await submitWithdrawal(poolAddress, parsedProof, root, nullifierHash, recipient, parsedFee);
