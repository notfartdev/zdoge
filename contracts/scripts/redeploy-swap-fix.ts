...
/**
 * Redeploy SwapVerifier and ShieldedPoolMultiTokenV2 to fix swap zkey mismatch
 * 
 * This script:
 * 1. Deploys a new SwapVerifier (generated from current zkey)
 * 2. Redeploys ShieldedPoolMultiTokenV2 with the new SwapVerifier
 * 3. Keeps all other verifiers unchanged (shield/transfer/unshield continue working)
 * 
 * ⚠️ IMPORTANT: This creates a NEW pool contract. Old pool remains with existing balances.
 * Users must unshield from old pool and shield into new pool to use swap.
 * 
...
async function main() {
  ...
  console.log("ZDOGE - Swap Fix: Redeploy SwapVerifier + ShieldedPoolMultiTokenV2");
  ...
  // ============ Step 1: Deploy New SwapVerifier ============
  
  console.log("\n" + "=".repeat(80));
  console.log("STEP 1: Deploying New SwapVerifier");
  console.log("=".repeat(80));
  console.log("\n⚠️  Make sure you've generated SwapVerifier.sol from the current zkey!");
  console.log("   Run: circuits/shielded/regenerate-swap-verifier.ps1");
  console.log("   This ensures the verifier matches the current swap_final.zkey\n");
  ...
  const NEW_SWAP_VERIFIER = await swapVerifier.getAddress();
  
  console.log("✅ New SwapVerifier deployed to:", NEW_SWAP_VERIFIER);
  ...
  // Reuse existing verifiers (shield, transfer, unshield are unchanged)
  ...
  // OLD_SWAP_VERIFIER = "0xE264695FF93e2baa700C3518227EBc917092bd3A" (replaced)
  ...
  console.log("├─ Unshield Verifier:    ", UNSHIELD_VERIFIER, " (unchanged)");
  console.log("├─ Swap Verifier:        ", NEW_SWAP_VERIFIER, " ⭐ NEW");
  ...
  // ============ Step 3: Deploy New ShieldedPoolMultiTokenV2 ============
  
  console.log("\n" + "=".repeat(80));
  console.log("STEP 3: Deploying New ShieldedPoolMultiTokenV3");
  console.log("=".repeat(80));
  console.log("\n⚠️  This creates a NEW pool contract with a NEW address.");
  console.log("   Old pool at 0x6FdE88710d7D8974A040064a294A6a9aAE956D0F will remain.");
  ...
  const shieldedPool = await ShieldedPoolV3.deploy(
    ...
    TRANSFER_VERIFIER,
    UNSHIELD_VERIFIER,
    NEW_SWAP_VERIFIER,  // NEW: Updated swap verifier
  ...
  console.log("✅ New ShieldedPoolMultiTokenV2 deployed to:", NEW_POOL_ADDRESS);
  ...
  console.log("└─ Swap Verifier:    ", swapVerifierAddr, swapVerifierAddr === NEW_SWAP_VERIFIER ? "✅" : "❌");
  ...
  console.log("SwapVerifier:            ", NEW_SWAP_VERIFIER);
  
  console.log("\n📝 Old Contract Addresses (still exists):");
  console.log("Old ShieldedPoolMultiTokenV3: 0x6FdE88710d7D8974A040064a294A6a9aAE956D0F");
  console.log("Old SwapVerifier:             0xE264695FF93e2baa700C3518227EBc917092bd3A");
  
  console.log("\n🔗 Verification Commands:");
  console.log(`# Verify SwapVerifier:`);
  console.log(`npx hardhat verify --network dogeosTestnet ${NEW_SWAP_VERIFIER}`);
  ...
  console.log(`New SwapVerifier: https://blockscout.testnet.dogeos.com/address/${NEW_SWAP_VERIFIER}`);
  ...
  console.log("\n3. Update backend/render.yaml (if using):");
  console.log(`   SHIELDED_POOL_ADDRESS: ${NEW_POOL_ADDRESS}`);
  ...
  console.log("   ✅ Unshield (should work)");
  console.log("   ✅ Swap (should work now!)");
  console.log("\n7. Verify swap works correctly");
  console.log("\n" + "=".repeat(80));
  console.log("🎉 Swap should now work! All other operations continue working.");
  console.log("=".repeat(80));
  
  return {
    newPoolAddress: NEW_POOL_ADDRESS,
    newSwapVerifier: NEW_SWAP_VERIFIER,
  };
}
