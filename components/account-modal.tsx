          ...
          <div className="p-4 space-y-4 overflow-y-auto flex-1 min-h-0">
              ...
              <div className="p-4 bg-black border border-[#C2A633]/30 space-y-3">
                ...
                <div className="flex items-center justify-between bg-zinc-900 p-3 border border-zinc-800">
                  <div className="flex-1 min-w-0">
                    <p className="font-body text-xs text-white/60 mb-1">Address</p>
              ...
              <div className="p-4 bg-black border border-zinc-800 space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <button 
                    ...
                    className="text-[10px] font-body text-[#C2A633] hover:underline"
                  >
                    Refresh
                  </button>
                </div>
                ...
                <div className="space-y-2">
                  {loadingBalances ? (
                    <div className="text-center py-3">
                      <span className="font-body text-xs text-white/60">Loading balances...</span>
                    </div>
                    ...
                    tokenBalances.map((token, i) => (
                      <div key={i} className="flex items-center justify-between py-2 border-b border-zinc-800 last:border-0">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-body text-sm text-white/70">{token.symbol}</span>
                        </div>
                    ...
                    <div className="text-center py-3">
                      <span className="font-body text-xs text-white/60">No balances found</span>
                    </div>
                  )}
                </div>
              </div>
            ...
            <div className="p-4 bg-black border border-zinc-800 space-y-3">
              ...
              <div className="flex items-center justify-between bg-zinc-900 p-3 border border-zinc-800">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-body text-sm text-white">{dogeosTestnet.name}</p>
                    <p className="font-body text-[10px] text-white/60">Chain ID: <span className="font-mono">{dogeosTestnet.id}</span></p>
                  </div>
                </div>
            ...
            <div className="p-4 bg-black border border-zinc-800 space-y-3">
              ...
              <div className="relative">
                ...
                >
                  <span className="font-body text-sm text-white">{selectedRpc.name}</span>
                  ...
                  <div className="absolute top-full left-0 right-0 mt-1 bg-zinc-900 border border-zinc-700 z-10 shadow-lg">
                    {RPC_OPTIONS.map((rpc) => (
                      <button
                        ...
                        className={`w-full text-left p-3 font-body text-sm hover:bg-[#C2A633]/20 transition-colors flex items-center justify-between ${
              ...
              {selectedRpc.id === 'custom' && (
                <div className="space-y-2">
                  <input
                    ...
                    className="w-full p-3 bg-zinc-800 border border-zinc-700 font-body text-sm text-white placeholder:text-gray-500 focus:border-[#C2A633]/50 focus:outline-none"
                  />
                  <Button
            ...
            {wallet?.isConnected && (
              <Button
                ...
                onClick={handleSync}
                disabled={isSyncing}
                className="w-full font-body text-sm border-[#C2A633]/50 text-[#C2A633] hover:bg-[#C2A633]/10 hover:text-[#C2A633] flex items-center justify-center gap-2"
              >
